...
-- The total berth length at MIT is 1219.2 m.
...
-- Total berth capacity is the product of 1219200 mm and 31 days = 37795200 mm-days
-- 1219200 * 31 = 37795200
-- 1219200 * 30 = 36576000
-- 1219200 * 28 = 34137600
...
FROM
	...
	SELECT
		...
		(vc.loa+50000)*(vv.atd-vv.ata)/34137600*100 AS BerthUtil
	FROM 
		mtms.vessels msv
	JOIN spinnaker.vessel spv ON spv.code=msv.id
	JOIN spinnaker.VC_CLASS vc ON spv.CLASS_ID=vc.id
	...
	SELECT
		...
		CASE 
			WHEN 	(EXTRACT (MONTH FROM vv.atd)=1 OR EXTRACT (MONTH FROM vv.atd)=3 OR EXTRACT (MONTH FROM vv.atd)=5 OR EXTRACT (MONTH FROM vv.atd)=7
				...
				THEN (vc.loa+50000)*(vv.atd-vv.ata)/1219200/31
			WHEN	(EXTRACT (MONTH FROM vv.atd)=4 OR EXTRACT (MONTH FROM vv.atd)=6 OR EXTRACT (MONTH FROM vv.atd)=9 OR EXTRACT (MONTH FROM vv.atd)=11)
				THEN (vc.loa+50000)*(vv.atd-vv.ata)/1219200/30
			WHEN	(EXTRACT (MONTH FROM vv.atd)=2)
				THEN (vc.loa+50000)*(vv.atd-vv.ata)/1219200/28
